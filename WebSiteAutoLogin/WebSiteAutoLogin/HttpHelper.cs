...
namespace Http
{
    public static class HttpHelper
    {
        ...
        /// <returns></returns>  
        public static string Get(string url, int? timeout = null, string userAgent = null, CookieCollection cookies = null, string contentType = null, IList<ParamKeyValue> heards = null,Encoding encoding=null)
        {
            ...
            try
            {
                ...
                var result = GetTxtByHttpWebResponse(httpResponse, true, encoding);
        ...
        public static HttpWebRequest CreateHttpResponse(string url, int? timeout, string userAgent, CookieCollection cookies, string contentType, IList<ParamKeyValue> heards)
        {
            ...
            request.ContentType = !string.IsNullOrWhiteSpace(contentType) ? contentType : "application/x-www-form-urlencoded";
        ...
        public static string ParamList2Str(ICollection<ParamKeyValue> parameters)
        {
            ...
            foreach (var parameter in parameters)
            {
                ...
                buffer.AppendFormat("{0}={1}", HttpUtility.UrlEncode(parameter.Key), HttpUtility.UrlEncode(parameter.Value));
            }
        ...
        /// <returns></returns>
        public static string GetTxtByHttpWebResponse(HttpWebResponse hwr, bool closeHttpWebResponse = false,Encoding encoding=null)
        {
            var stream = hwr.GetResponseStream();
            var reader = new StreamReader(stream,encoding == null ? Encoding.GetEncoding(hwr.CharacterSet) : encoding);
