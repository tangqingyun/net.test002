...
<table id="tt" style="width:100%;height:auto; border="0" >
    ...
    <tbody id="tbody">
        <asp:Repeater ID="rpData" runat="server">
            <ItemTemplate>
                 <tr>
                    <td><input type="checkbox" name="arcID" value="<%#Eval("ArcticleID") %>" /></td>
    ...
    <tr>
        <td colspan="10">
            <a id="selAll" href="#" class="easyui-linkbutton">全选</a>&nbsp;&nbsp;
            <a id="noSelAll" href="#" class="easyui-linkbutton">取消</a>&nbsp;&nbsp;
            <a id="delArc" href="#" class="easyui-linkbutton">删除</a>&nbsp;&nbsp;
            <a id="adArc" href="#" class="easyui-linkbutton">推荐</a>&nbsp;&nbsp;
        </td>
    </tr>
     <tr class="pager">
     <td colspan="10">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#selAll").click(function () {
                $("input[name='arcID']").attr("checked",true);
            });

            $("#noSelAll").click(function () {
                $("input[name='arcID']").attr("checked", false);
            });

            $("#delArc").click(function () {
                var arrid = getSelectedArcId();
                if (arrid.length == 0) {
                    alert("请选择要删除的文章");
                    return false;
                }
                if (!confirm("确定要删除选中文章吗？")) {
                    return false;
                }
                $.post("/Ashx/arcticle_manage.ashx", { "ACTION": "delArc", "AId": arrid.toString() }, function (data) {
                    if (data == "True") {
                        alert("删除成功");
                        location.reload();
                        return false;
            ...
            $("#adArc").click(function () {
                var arrid = getSelectedArcId();
                if (arrid.length == 0) {
                    alert("请选择要推荐的文章");
                    return false;
                }
                $.post("/Ashx/arcticle_manage.ashx", { "ACTION": "adArc", "AId": arrid.toString()}, function (data) {
                    if (data == "True") {
                        alert("推荐成功");
                        return false;
                    }
        ...
        function getSelectedArcId() {
            var arrid = [];
            $("input[name='arcID']:checked").each(function (i) {
                arrid[i] = $(this).val();
            });
            return arrid;
        }
